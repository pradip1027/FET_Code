...
let box = document.getElementById("containerm");

function createproductcard(productObj){
    let card = document.createElement("div");

    card.setAttribute("class","card");
    card.classList.add("cardA","cardB");
    ...
    pic.src = productObj.images[0];
    pic.setAttribute("alt", `This is ${productObj.title}`);

    card.append(name, pic);
    box.append(card);

}


function createAllcards(productArr){
    productArr.forEach(e => {
        createproductcard(e);
    });
}

createAllcards(ecom.products);