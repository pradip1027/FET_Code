...
function createcard(personobj){

    let card = document.createElement("div");
    card.setAttribute("class","card");

    let id  = document.createElement("p");
    id.innerText = personobj.id;
    id.style.fontSize = "20px";

    let name = document.createElement("h3");
    name.innerText = personobj.firstName + " " + personobj.lastName;

    let gender = document.createElement("p");
    gender.innerText = personobj.gender;
}

function createcards(personarry){
    for(let i = 0; i < personarry.length; i++){
        createcard(personarry[i])
    }
}
